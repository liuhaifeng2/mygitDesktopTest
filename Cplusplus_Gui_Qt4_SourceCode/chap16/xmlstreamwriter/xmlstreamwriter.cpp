...
//    treeWidget->header()->setResizeMode(QHeaderView::Stretch);        // for Qt4
    treeWidget->header()->setSectionResizeMode(QHeaderView::Stretch);   // for Qt5
    treeWidget->setWindowTitle(QObject::tr("XML Stream Writer"));
    treeWidget->show();

    // (void) 抑制编译器警告: 告诉编译器此表达式无需返回值
